...
def main():
	...
	for lvl_key, sentences in data_generator:
		...
		for sentence in sentences:
			# filter out irrelevant sentences (if using subset)
			if (relevant_idcs is not None) and (sentence.idx not in relevant_idcs): continue
			# append sentence and genres
		...
		if len(genres) < 2:
			...
			pred_dist = np.ones((len(sentences), 1))
		# run clustering
		else:
			...
			# run LDA
			domains, pred_dist = run_lda(corpus, vectorizer, len(genres), random_state=args.seed)
		...
		if relevant_idcs is not None:
			pred_idx = 0
			for lvl_sen_idx, sen in enumerate(sentences):
				if sen.idx not in relevant_idcs: continue
